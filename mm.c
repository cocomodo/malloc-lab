...
team_t team = {
    /* Team name */
    "KRAFTON JUNGLE_Team 08",
    /* First member's full name */
    "HYEOK CHANG",
    /* First member's email address */
    "programmingonly17@gmail.com",
...
//메모리 할당 시 기본적으로 header와 footer를 위해 필요한 더블워드 만큼의 메모리 크기
...
#define CHUNKSIZE (1<<12) //extend haep by this amount : 4096bytes->4kib
...
int mm_init(void)
{
    ...
    PUT(heap_listp + (2*WSIZE), NULL); //prolouge block안의 PRED 포인터 NULL로 초기화
    PUT(heap_listp + (3*WSIZE), NULL); //prolouge block안의 SUCC 포인터 NULL로 초기화
...
static void* extend_heap(size_t words){ //워드 단위로 받는다. 
    ...
    if ((long)(bp = mem_sbrk(size)) == -1) //새 메모리의 첫 부분을 bp로 둔다. 주소값은 int로는 못 받아서 long 으로 casting 하는듯.
        return NULL;
    
    //새 가용 블록의 headr와 footer를 정해주고 epilogue block을 가용 블랙 맨 끝으로 옮긴다.
    PUT(HDRP(bp), PACK(size, 0)); //header에 size 만큼 가용하다는 정보 삽입.
    PUT(FTRP(bp), PACK(size, 0)); //footer에 size 만큼 가용하다는 정보 삽입.
...
void *mm_malloc(size_t size) 
{
    ...
    extendsize = MAX(asize, CHUNKSIZE); // 기본 힙 확장 사이즈는 CHUNGSIZE이므로 비교 후에 둘 중 더 큰 값으로 사이즈를 정함
...
static void* find_fit(size_t asize){
    ...
    /*free list의 맨 뒤(마지막)는 프롤로그 블록이다. Free list에서 유일하게 할당된(GET_ALLOC=1) 
    블록이므로 만나면 탐색 종료
    */
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    //원래 메모리 크기(copySize)보다 적은 크기(size)에 대해 재할당 요청이 오면 요청한 크기 만큼의
    //메모리만 할당
    if (size < copySize)
      copySize = size;
